function solve() {
  ...
  function createRowAndAppend(item) {
    ...
    tr.innerHTML += createElement(`<img src=${item.img}>`);
    tr.innerHTML += createElement(`<p>${item.name}</p>`);
    tr.innerHTML += createElement(`<p>${item.price}</p>`);
    tr.innerHTML += createElement(`<p>${item.decFactor}</p>`);
    tr.innerHTML += createElement(`<input type="checkbox">`);
