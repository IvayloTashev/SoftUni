...
userRouter.post('/register', isGuest(),
    body('email').trim().isLength({ min: 10 }).withMessage('Email must be at least 2 charecters logn'),
    body('username').trim().isLength({ min: 2 }).withMessage('Username must be at least 2 charecters logn'),
    body('password').trim().isLength({ min: 4 }).withMessage('Password must be at least 4 charecters logn'),
